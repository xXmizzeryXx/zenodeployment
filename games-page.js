...
function zasAdd(btn,id){if(games.some(g=>g.id==='zas-'+id)){closeAddModal();openGameModal(games.find(g=>g.id==='zas-'+id));return;}const name=fmt(id);const entryPath=id==='cine-cloud'?'https://flux-worker.eclipseservice.workers.dev/fetch/aHR0cHM6Ly9jY2NpaWlubm5lZWUuYi1jZG4ubmV0Lw==':'games/'+id+'.html';
  const entry={id:'zas-'+id,name,icon:null,entryPath,fileCount:0,fileRecords:null,r2:false,zenoapp:true};games.push(entry);saveGameToDB(entry).catch(console.error);window._fbSyncGame?.(entry);btn.textContent='Added';btn.className='zas-get added';renderGrid();showToast(name.toUpperCase()+' ADDED');}
