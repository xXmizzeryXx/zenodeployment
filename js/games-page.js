...
async function initSW() {
  ...
  try {
    ...
    for (const r of regs) {
      ...
      if (url.includes('zeno-game-sw.js')) await r.unregister();
    }
    const reg = await navigator.serviceWorker.register('./zeno-game-sw.js', { scope: './' });
...
function renderGrid() {
  ...
  if(!games.length){empty.style.display='flex';grid.style.display='none';fb.style.display='none';const gc=document.getElementById('gameCount');if(gc)gc.textContent='';syncGamesStats();document.getElementById('recentRow').style.display='none';if(favRowEl)favRowEl.style.display='none';return;}
...
function zasAdd(btn, id) {
  ...
  games.push(entry);
  saveGameToDB(entry).catch(console.error);
...
// ── INDEXEDDB ────────────────────────────────────────────────────
const DB_NAME='zeno-games-db',DB_VERSION=2,STORE_META='game-meta',STORE_FILES='game-files';
